...
// JmdictTypes represents the root structure of the JMdict file
...
type Sense struct {
	...
	AppliesToKanji []string         `json:"appliesToKanji,omitempty"`
	AppliesToKana  []string         `json:"appliesToKana,omitempty"`
	Related        [][]Xref         `json:"related,omitempty"`
	Antonym        [][]Xref         `json:"antonym,omitempty"`
	Field          []string         `json:"field,omitempty"`
	Dialect        []string         `json:"dialect,omitempty"`
	Misc           []string         `json:"misc,omitempty"`
	Info           []string         `json:"info,omitempty"`
	LanguageSource []LanguageSource `json:"languageSource,omitempty"`
...
type Gloss struct {
	Lang   string  `json:"lang"`
	Gender *string `json:"gender,omitempty"` // can be "masculine", "feminine", "neuter" or null
	Type   *string `json:"type,omitempty"`   // can be "literal", "figurative", "explanation", "trademark" or null
	Text   string  `json:"text"`
}
...
func (x *Xref) MarshalJSON() ([]byte, error) {
...
// Implement common.Entry interface
func (w Word) GetID() string {
	return w.ID
}

func (w Word) GetFilename() string {
	if len(w.Kanji) > 0 {
		return w.Kanji[0].Text
	}
	if len(w.Kana) > 0 {
		return w.Kana[0].Text
	}
	return w.ID
}

// SanitizeWildcards removes ["*"] wildcards from appliesToKanji and appliesToKana if they exist
// This should be called before serializing to JSON to reduce output size
func (s *Sense) SanitizeWildcards() {
	// If appliesToKanji is just ["*"], make it empty so it will be omitted
	if len(s.AppliesToKanji) == 1 && s.AppliesToKanji[0] == "*" {
		s.AppliesToKanji = []string{}
	}

	// If appliesToKana is just ["*"], make it empty so it will be omitted
	if len(s.AppliesToKana) == 1 && s.AppliesToKana[0] == "*" {
		s.AppliesToKana = []string{}
	}
}

// SanitizeWildcards removes ["*"] wildcards from appliesToKanji if they exist
// This should be called before serializing to JSON to reduce output size
func (k *KanaEntry) SanitizeWildcards() {
	// If appliesToKanji is just ["*"], make it empty so it will be omitted
	if len(k.AppliesToKanji) == 1 && k.AppliesToKanji[0] == "*" {
		k.AppliesToKanji = []string{}
	}
}
